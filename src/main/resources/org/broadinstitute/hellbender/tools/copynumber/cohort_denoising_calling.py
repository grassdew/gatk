...
if __name__ == "__main__":
    ...
    try:
        ...
        random.seed(warm_up_inference_params.random_seed)
        new_random_seed = random.randint(0, 2 ** 31 - 1)
        warm_up_inference_params.random_seed = new_random_seed
        warm_up_task = gcnvkernel.CohortDenoisingAndCallingWarmUpTask(
            warm_up_denoising_config, warm_up_inference_params,
            shared_workspace, initial_params_supplier)
    ...
    try:
        ...
        logger.info("Retrying inference one more time...")
        # Make sure we have reproducible results
        random.seed(main_inference_params.random_seed)
        new_random_seed = random.randint(0, 2 ** 31 - 1)
        main_inference_params.random_seed = new_random_seed
        main_task = gcnvkernel.CohortDenoisingAndCallingMainTask(
            main_denoising_config, main_calling_config, main_inference_params,
            shared_workspace, initial_params_supplier, warm_up_task)
